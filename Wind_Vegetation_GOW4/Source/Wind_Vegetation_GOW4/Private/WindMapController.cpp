...
void UWindMapController::UpdateOwnedWindMap()
{
	for (UWindMap* windMap : OwnedWindMap)
	{
		if(IsValid(windMap))
		{
			...
			windMap->WindMapTransformMatrix = FTranslationMatrix::Make(FVector(-0.5f, -0.5f, -0.5f)) * FScaleMatrix::Make(WindMapSizeInWorldSpace) * FRotationMatrix::Make(GetWindMapRotation()) * FTranslationMatrix::Make(WindMapOffsetInWorldSpace) * FTranslationMatrix::Make(GetWindMapOriginWorldPosition());
		}
	}
}
